...
def pytest_addoption(parser):
    ...
    group._addoption(
        "--headless",
        metavar="str",
        help="Is selenium in headless mode when starting",
    )
    # group._addoption(
    #     "--selenium-port",
    #     type=int,
...
def driver_class(pytestconfig):
    ...
    if headless == 'headless':
        ...
        # 禁用GPU，防止无头模式出现莫名的bug 2023-12-12
        options.add_argument('--disable-gpu')

    if driver_argv == "REMOTE":
        host = pytestconfig.getoption("host")
        port = pytestconfig.getoption("port")
        remote_address = "http://{}:{}/wd/hub".format(host, str(port))
        print('grid_url=', remote_address)
        driver = webdriver.Remote(
            command_executor=remote_address,
            desired_capabilities=options.to_capabilities()
        )
    else:  # 2023-12-12 修改为不论本地与远程，都可以使用无头或非无头模式进行测试
        driver = SUPPORTED_DRIVERS[driver_argv](options=options)
    return driver
